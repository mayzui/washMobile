...
export default {
  ...
  data() {
    ...
    var validateWechat = (rule, value, callback) => {
      if(!value && this.form.is_login_wechat == 1){
  ...
  methods: {
    ...
    submit(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$api.info.addaccount(this.form).then((data) => {
            if (data.code == 200) {
              this.$message.success("添加成功");
              this.goBack();
            } else {
              this.$message.error(data.msg ? data.msg : "添加失败");
            }
          });
        } else {
